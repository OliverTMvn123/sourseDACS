...
if(empty($nguoinhan)|| empty($address)||empty($noidung) ||empty($username) ||empty($password) ||empty($sdt) )
{
    echo "<script>
    ...
    if($flag==1)
    {
        //thêm thư vào hòm thư admin
    $sql="INSERT INTO `mail`( `vandelienquan`, `nguoigui`, `sdt`, `diachi`,`email`, `chude`, `noidung`, `anh`, `sendTime`) 
    VALUES ('$nguoinhan','$nguoigui','$sdt','$address','$email','$Chude','$noidung','$attachment1','$sendtime1')";
                if($conn->query($sql)===TRUE)
                {
                    ...
                    print_r($ID);
                    //thêm vào mailNguoi dùng
                    $sql1="INSERT INTO `mailnguoidung`(`ID`, `nguoigui`, `nguoinhan`, `SDT`, `DIACHI`, `noidung`, `vandelienquan`, `Email`, `sendTime`, `anh`,`mailID`,`chude`) 
                    VALUES ('','$nguoigui','ADMIN','$sdt','$address','$noidung','$nguoinhan','$email','$sendtime1','$attachment1','$ID','$Chude')";
