<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title> GIAO DIỆN NGƯỜI DÙNG </title>
    <link rel="stylesheet" href="./formxemthu.css">


</head>

<body>
    <header>
        <div id="logo1" s>
            <img id="logo" src="../homepage/logo.png" alt="">
        </div>
        <div id="title1">
            <h1> Cổng Tiếp Nhận Phản Hồi Ý Kiến Của Người Dân</h1>
        </div>
        <div style="float:right" id="avatarcontroll">
            <a id="avatarcontroll1" href="../formNguoidung/frmUser.php"> <img id="avatar"
                    src="data:image/jpeg;base64,"
                    alt=""></a>
        </div>
        <div class="clear"></div>
    </header>
    <main>
        <div id="menu">
            <ul>
                <li id="listmenu">
                    <a href=""><img id="menuicon" src="menuicon.png" alt=""></a>
                    <ul id="showmenu">
                        <li><a href="../homepage/DACS.html"> Trang Chủ</a></li>
                        <hr>
                        <li><a href="../Mail/mail.html"> Gửi Thư</a></li>
                        <hr>
                        <li><a href="./homthucanhan/homthuUser.html"> Hòm thư cá nhân </a></li>
                        <hr>
                        <li><a href=""> NỘI DUNG 1</a></li>
                        <hr>
                        <li id="licuoi"><a href="../homepage/DACS.html"> Đăng Xuất</a></li>

                    </ul>
                </li>
            </ul>
        </div>

        <div id=nav>

            <head>
                <h1 align="center" id="Tieude"> Hòm thư đến </h1>
                <div id='back'>
                    <a href="./frmgiaodien.php"><img id='backicon' src="./backicon.png" alt=""></a>
                </div>
            </head>
            <hr>
            <div id="Select-Mail">
                
                <?php
                    require '../homepage/ConnectDB.php';
                        $sql='SELECT * FROM `mail`';
                        $result=$conn->query($sql);
                            $i=0;
                        while($row= $result->fetch_assoc())
                        {
                                $list= array($row['vandelienquan'],$row['nguoigui'],$row['diachi'],$row['email'],$row['sendTime'],$row['chude'],$row['noidung'],$row['anh'],$row['sdt'],$row['verification']);
                                $list1[$i]=$list;
                                $i++;
                        }
                  
                        echo'
                            <div class="khung">
                                <h4>Người gửi : '.$list1[$_POST['so']][1].'</h4>
                            </div>
                            <div  class="khung">
                                <h4>Chủ Đề Thắc Mắc:  '.$list1[$_POST['so']][0].'</h4>
                            </div >
                            <div  class="khung">
                                <h4>Thời gian gửi :'.$list1[$_POST['so']][4].' </h4>
                            </div>
                            <div  class="khung">
                                <h4>Số Điện Thoại :0'.$list1[$_POST['so']][8].' </h4>
                            </div>
                            <div class="khung">
                                <h4>Địa Chỉ : '.$list1[$_POST['so']][2].' </h4>
                            </div>
                            <div class="khung">
                            <h4>Email : '.$list1[$_POST['so']][3].' </h4>
                            </div>
                            <div class="khung">
                                <h4>Chủ Đề : '.$list1[$_POST['so']][5].'</h4>
                            </div>
                            <div class="khung">
                                <h4>Nội Dung :'.$list1[$_POST['so']][6].' </h4>
                            </div>
                            <div class="khung">
                                <h4>Ảnh :'.$list1[$_POST['so']][7].'</h4>
                                <img id=anh src="../sendMail/upload/'.$list1[$_POST['so']][7].'" alt="">
                                <img id=anh src="../sendMail//camera/upload/'.$list1[$_POST['so']][9].'" alt="">
                            </div>
                        ';

                            ?>
                </table>
               
            </div>
            <script>

            </script>
        </div>

        <div class="clear"></div>
    </main>
    <footer>

    </footer>
</body>

</html>